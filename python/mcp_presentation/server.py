...
@mcp.tool()
def create_workspace(project_id: str, path: str, ref_name: str = "main") -> dict:
...
@mcp.tool()
def set_active_workspace(session_id: str, workspace_id: str) -> dict:
    """Attach an active workspace to a session."""
    get_state().set_active_workspace(session_id, workspace_id)
    return get_state().get_session(session_id) or {
        "error": "not_found",
...
def build_presentation(session_id: str, target: str) -> dict:
    ...
    tid = get_tasks().submit(session_id, ws["path"], target)
    return {"task_id": tid, "status": "queued", "workspace": ws["path"]}
...
@mcp.tool()
def deploy_presentation(session_id: str, artifact: str = "") -> dict:
    ...
    if ws is None:
        return {"error": "workspace_unavailable", "workspace_id": wid}
    tid = get_tasks().submit(session_id, ws["path"], "deploy")
    if artifact:
